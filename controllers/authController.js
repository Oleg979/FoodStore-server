...
router.post("/register", (req, res) => {
  User.findOne({ email: req.body.email }, (err, user) => {
    if (user)
      return res.status(401).send({ auth: false, text: "Пользователь с такой почтой уже существует!" });
    else {
...
router.get("/me", VerifyToken, (req, res) => {
  User.findById(req.userId, { password: 0 }, (err, user) => {
    ...
    if (!user) return res.status(404).send("Пользователь не существует.");
...
router.post("/login", (req, res) => {
  User.findOne({ email: req.body.email }, (err, user) => {
    if (err)
      return res
        .status(500)
        .send({ auth: false, text: "Произошла ошибка на сервере, попробуйте позже." });
    if (!user)
      return res.status(404).send({ auth: false, text: "Пользователя с такой почтой не существует!" });
    ...
    if (!user.isEmailVerified)
      return res
        .status(401)
        .send({ auth: false, token: null, text: "Ваша почта не подтверждена. Проверьте почтовый ящик, мы отправили Вам код потверждения." });
...
router.post("/verify", (req, res) => {
  User.findOne({ email: req.body.email }, (err, user) => {
    if (err)
      return res
        .status(500)
        .send({ success: false, text: "Произошла ошибка на сервере, попробуйте позже." });
    if (!user)
      return res.status(404).send({ success: false, text: "Неверный адрес электронной почты." });
